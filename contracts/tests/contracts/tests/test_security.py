...
class SecuritySettingsTests(TestCase):
    ...
    def test_cors_allow_all_origins_is_not_true_after_day30(self):
        """
        After Day 30, CORS_ALLOW_ALL_ORIGINS should NOT be set
        at all — we use CORS_ALLOWED_ORIGINS exclusively.
        """
        cors_all = getattr(settings, 'CORS_ALLOW_ALL_ORIGINS', False)
        self.assertFalse(
            cors_all,
            "CORS_ALLOW_ALL_ORIGINS should be False after Day 30 — "
...
class CORSConfigurationTests(TestCase):
    ...
    def test_allowed_origin_gets_cors_header(self):
        ...
        # CORS headers are present when origin is sent
        self.assertIn(
...
class FileUploadSecurityTests(TestCase):
    ...
    def test_oversized_file_is_rejected(self):
        ...
        # Either 400 (caught by our validator) or 413 (caught
        # by Django's upload limit) — both mean rejected
        self.assertIn(
            response.status_code,
            [
    ...
    def test_sql_injection_in_search_is_safe(self):
        ...
        for query in malicious_queries:
            ...
            # Must return 200 (Django ORM parameterizes all
            # queries — injection impossible) with empty results
            self.assertEqual(
                response.status_code,
                200,
    ...
    def test_sql_injection_in_status_filter_is_safe(self):
        ...
        # Our validator rejects the invalid status value
        self.assertEqual(response.status_code, 400)